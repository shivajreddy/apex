...
run_as_admin = false     # elevated logon task; see below
...
and apex cannot take focus. `run_as_admin = true` fixes that by starting apex
from a scheduled logon task with highest privileges - no UAC prompt at
sign-in, just one when the task is created. It replaces the `Run` key, and
takes effect at the next sign-in.

Apps you launch are **not** elevated with it. A child process normally
inherits its parent's token, so an elevated launcher would silently start
everything as administrator - Windows never consults the target's manifest,
which only decides whether an *unelevated* parent needs to escalate. Apex
therefore hands the target to Explorer, which runs as you, so launching from
apex matches launching from the Start menu.

The one exception is a quicklink with `open_with`: Explorer cannot forward a
program-and-argument pair, so those launch directly and do inherit elevation.
